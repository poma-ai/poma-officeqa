...
try:
    from _branding import add_branding
except ImportError:
    def add_branding(path):  # noqa: E302
        pass
...
def plot_accuracy_curves_zoomed(output_path):
    ...
    # Fine-grained budgets up to 400K (to show POMA Chunksets reaching 100% at ~340K)
    ...
    # Plot order: baselines first, then POMA (so POMA is on top)
    plot_order = ["unstructured", "databricks_rcs", "poma_chunksets"]
    ...
    for method in plot_order:
        ...
        lw = 3.5 if method == "poma_chunksets" else LINE_WIDTHS[method]
        ax.plot(budgets, accuracies,
                linestyle=LINE_STYLES[method],
                color=COLORS[method],
                label=LABELS[method],
...
def plot_token_comparison_bars(output_path):
    ...
    # Percentage callout above the POMA bar
    poma_max = maxes[0]
    baseline_worst = max(maxes[1:])
    pct_less = (1 - poma_max / baseline_worst) * 100
    ax.text(0, maxes[0]/1000 + max(maxes)/1000 * 0.08,
            f'{pct_less:.0f}% fewer tokens',
            ha='center', fontsize=11, fontweight='bold',
            color=COLORS["poma_chunksets"])
...
def plot_token_comparison_compact(output_path):
    """Compact horizontal 2-bar chart for README hero section.

    Shows only POMA Chunksets vs the worse baseline — designed to be
    small, glanceable, and placed next to the headline.
    """
    poma_max = max(RESULTS["poma_chunksets"].values())
    db_max = max(RESULTS["databricks_rcs"].values())
    us_max = max(RESULTS["unstructured"].values())
    baseline_max = max(db_max, us_max)
    baseline_label = "Conventional" if us_max >= db_max else "Conventional"

    fig, ax = plt.subplots(figsize=(6, 2.2))

    bars = ax.barh(
        [baseline_label, "POMA"],
        [baseline_max / 1e3, poma_max / 1e3],
        color=["#e74c3c", "#2ecc71"],
        height=0.55,
    )

    # Value labels at end of bars
    for bar, val in zip(bars, [baseline_max, poma_max]):
        label = f'{val/1e6:.1f}M' if val >= 1e6 else f'{val/1e3:.0f}K'
        ax.text(bar.get_width() + 15, bar.get_y() + bar.get_height() / 2,
                label, va='center', fontsize=12, fontweight='bold')

    # "77% fewer tokens" badge in the whitespace
    pct_fewer = (1 - poma_max / baseline_max) * 100
    ax.text(poma_max / 1e3 + (baseline_max - poma_max) / 1e3 * 0.5,
            1.15,  # between the two bars
            f'{pct_fewer:.0f}% fewer tokens',
            ha='center', va='center', fontsize=13, fontweight='bold',
            color='#2ecc71',
            bbox=dict(boxstyle='round,pad=0.3', facecolor='#f0faf0',
                      edgecolor='#2ecc71', linewidth=1.5))

    ax.set_xlabel('Tokens needed for 100% recall', fontsize=10)
    ax.set_xlim(0, baseline_max / 1e3 * 1.18)
    ax.xaxis.set_major_formatter(plt.FuncFormatter(
        lambda x, p: f'{x/1e3:.1f}M' if x >= 1e3 else f'{x:.0f}K'))
...
def plot_token_heaps(output_path):
    ...
    # Color palette for questions (20 distinct colors)
    cmap = plt.colormaps.get_cmap('tab20')
    uid_colors = {uid: cmap(i / n_questions) for i, uid in enumerate(all_uids)}
    ...
    # Position heaps with a small fixed gap between them
    gap = 3
    x_offset = 0
    ...
    for method_idx, method in enumerate(methods):
        ...
        # Advance x_offset past this heap's actual width
        x_offset += grid_width + gap
...
def plot_stacked_context_waterfall(output_path):
    ...
    for method in methods:
        ...
        cumulative_data[method] = cumulative

        lw = 3.5 if method == "poma_chunksets" else 2.5
        ax.plot(range(len(all_uids)), cumulative, color=COLORS[method],
               linewidth=lw, linestyle=LINE_STYLES[method],
               label=LABELS[method])

    # Fill only the gap between POMA and worst baseline to show "wasted tokens"
    if "poma_chunksets" in cumulative_data and "unstructured" in cumulative_data:
        worst = np.maximum(cumulative_data.get("databricks_rcs", 0),
                          cumulative_data.get("unstructured", 0))
        ax.fill_between(range(len(all_uids)),
                       cumulative_data["poma_chunksets"], worst,
                       alpha=0.12, color='#e74c3c', label='Wasted tokens')

    ax.set_xlabel('Questions (sorted by POMA token requirement)', fontsize=12)
    ax.set_ylabel('Cumulative Tokens', fontsize=12)
    ax.set_title('Cumulative Context Tokens: How Costs Accumulate\n(Questions sorted easiest → hardest for POMA — shaded area = wasted tokens)',
                fontsize=14, fontweight='bold')
...
def main():
    ...
    plot_token_comparison_compact(output_dir / "token_comparison_compact.png")
